...
export default function WishlistPage() {
  const dispatch   = useDispatch();
  const items      = useSelector(selectWishlistItems);  // ← real Redux

  const [viewMode,  setViewMode]  = useState("grid");
  const [sortBy,    setSortBy]    = useState("recent");
  const [filter,    setFilter]    = useState("all");
  const [lastRemoved, setLastRemoved] = useState(null); // for undo snackbar
  const [addedIds,  setAddedIds]  = useState([]);       // moved-to-cart ids
const products = useSelector(selectProducts);
  ...
  const filtered = items
    ...
    .sort((a, b) => {
      ...
      if (sortBy === "price-high") return b.price - a.price;
      if (sortBy === "rating")     return (b.rating ?? 0) - (a.rating ?? 0);
      // "recent" — sort by addedOn descending
      return new Date(b.addedOn ?? 0) - new Date(a.addedOn ?? 0);
    });

  const inStockCount    = items.filter((i) => (i.stock ?? 1) > 0).length;
  const outOfStockCount = items.length - inStockCount;
  const maxDisc         = items.length
    ? Math.max(...items.map((i) => discPct(i.price, i.originalPrice) ?? 0))
    : 0;
  const totalSaving     = items.reduce((s, i) =>
    s + Math.max(0, (i.originalPrice ?? i.price) - i.price), 0);
  ...
  const handleMoveAll = () => {
    items
      .filter((i) => (i.stock ?? 1) > 0)
      .forEach((i) => handleMoveToCart(i));
  };
...
const recommendations = products
  ...
  .filter(p =>
    items.some(w =>
      w.mainCategory === p.mainCategory ||
      w.subCategory === p.subCategory ||
      (w.tags ?? []).some(tag => (p.tags ?? []).includes(tag))
    )
  )
  .sort((a, b) => {
    const score = (p) =>
      items.some(w => w.subCategory === p.subCategory) ? 2 :
      items.some(w => w.mainCategory === p.mainCategory) ? 1 : 0;
  ...
  if (items.length === 0) return (
    <div className="min-h-screen bg-[#FEF9F5] flex flex-col items-center justify-center py-20 px-4 text-center">
      ...
      <p className="text-base text-rose-900/50 mb-8 max-w-sm leading-relaxed">
        Tap the ♥ on any product to save it here. Never lose track of a hamper you love.
      </p>
      <Link to="/products"
        className="inline-flex items-center gap-2 px-10 py-3.5 rounded-full font-bold text-base text-white hover:shadow-xl hover:-translate-y-0.5 transition-all"
        style={{ background: "linear-gradient(135deg,#C2556A,#E8956D)" }}>
        ...
        <div className="relative z-10 max-w-7xl mx-auto">
          <div className="flex items-end justify-between flex-wrap gap-4">
            <div>
              ...
              <p className="text-white/60 text-sm mt-1.5">
                ...
                {totalSaving > 0 && (
                  <> · Savings potential:{" "}
                    <span className="text-white font-bold">₹{totalSaving.toLocaleString()}</span>
                  </>
                )}
              </p>
            </div>
            ...
            {inStockCount > 0 && (
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 rounded-full font-bold text-sm border border-white/25 text-white hover:bg-white/10 transition-all whitespace-nowrap"
              >
                <ShoppingCart size={15} />
                Move All to Cart ({inStockCount})
              </button>
          ...
          <div className="flex flex-wrap gap-2.5 mt-5">
            {[
              { icon: <Heart size={13} />,    label: `${items.length} Saved`      },
              { icon: <Package size={13} />,  label: `${inStockCount} In Stock`   },
              { icon: <Tag size={13} />,      label: `Up to ${maxDisc}% off`      },
              { icon: <Sparkles size={13} />, label: `${items.filter((i) => i.isFeatured).length} Bestsellers` },
            ].map(({ icon, label }) => (
              <div key={label}
                className="flex items-center gap-1.5 text-xs font-semibold text-white px-3 py-1.5 rounded-full border border-white/20"
      ...
      <AnimatePresence>
        {lastRemoved && (
          <motion.div
            initial={{ y: 80, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
            exit={{ y: 80, opacity: 0 }}
            className="fixed bottom-24 left-1/2 -translate-x-1/2 z-50 flex items-center gap-4 bg-[#3B2A35] text-white px-5 py-3 rounded-2xl shadow-2xl text-sm font-medium whitespace-nowrap"
          >
            <Trash2 size={15} className="text-rose-300 shrink-0" />
            <span>Removed from wishlist</span>
            <button onClick={handleUndo}
              className="font-bold text-[#E8956D] hover:text-[#C2556A] transition-colors ml-2 shrink-0">
              UNDO
            </button>
          </motion.div>
        )}
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-5 pb-16">
        ...
        <div className="bg-white rounded-2xl border border-rose-100 shadow-sm mb-5 overflow-hidden">
          ...
          <div className="px-4 pt-4 pb-3 flex gap-2 overflow-x-auto scrollbar-hide">
            {allTags.map((tag) => (
              <button key={tag} onClick={() => setFilter(tag)}
                className={`shrink-0 px-4 py-1.5 rounded-full text-sm font-semibold border transition-all
                  ${filter === tag ? "text-white border-transparent" : "text-rose-900/50 border-rose-200 bg-white hover:border-[#C2556A] hover:text-[#C2556A]"}`}
                style={filter === tag ? { background: "linear-gradient(135deg,#C2556A,#E8956D)" } : {}}>
                {tag === "all" ? `All Items (${items.length})` : tag}
              </button>
            ))}
          </div>
          ...
          <div className="flex items-center justify-between px-4 pb-3.5 border-t border-rose-50 pt-3 gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-1 border border-rose-200 rounded-xl p-1">
              {[{ mode: "grid", icon: <Grid3X3 size={15} /> }, { mode: "list", icon: <List size={15} /> }]
                .map(({ mode, icon }) => (
                  <button key={mode} onClick={() => setViewMode(mode)}
                    className={`p-1.5 rounded-lg transition-all
                      ${viewMode === mode ? "text-white" : "text-rose-900/40 hover:text-[#C2556A]"}`}
        ...
        <p className="text-sm text-rose-900/50 font-medium mb-4 px-1">
          Showing <span className="font-bold text-[#3B2A35]">{filtered.length}</span> items
          {filter !== "all" && <> in <span className="text-[#C2556A] font-bold">"{filter}"</span></>}
        </p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            <AnimatePresence>
              {filtered.map((item, index) => {
                const disc     = discPct(item.price, item.originalPrice);
                const inStock  = (item.stock ?? 1) > 0;
                const lowStock = inStock && (item.stock ?? 99) > 0 && (item.stock ?? 99) <= 5;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative aspect-square bg-[#FFF8F6] overflow-hidden">
                      ...
                      {item.isFeatured && (
                        <span className="absolute top-2 right-9 text-[10px] font-bold text-white bg-[#D4A847] px-2 py-0.5 rounded-full">
                          ★ Best
                        </span>
                      )}
                    ...
                    <div className="p-3 flex-1 flex flex-col">
                      ...
                      <div className="flex items-baseline gap-1.5 flex-wrap mt-auto mb-1">
                        <span className="text-base font-extrabold text-[#3B2A35]">₹{item.price.toLocaleString()}</span>
                        {item.originalPrice && item.originalPrice > item.price && (
                          <span className="text-xs text-rose-900/35 line-through">₹{item.originalPrice.toLocaleString()}</span>
                        )}
                      </div>

                      {item.addedOn && (
                        <p className="text-[10px] text-rose-900/30 mb-2">{timeAgo(item.addedOn)}</p>
                      )}

                      <div className="flex flex-wrap gap-1 mb-3">
                        {(item.tags ?? []).slice(0, 2).map((tag) => (
                          <span key={tag} className="text-[10px] font-semibold bg-[#FFF0F3] text-[#C2556A] border border-rose-100 px-1.5 py-0.5 rounded-full">
                            {tag}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="px-3 pb-3">
                      <button
                        onClick={() => inStock && handleMoveToCart(item)}
                        disabled={!inStock}
                        className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold transition-all
                          ${inStock ? "text-white hover:shadow-md hover:-translate-y-0.5" : "bg-gray-100 text-gray-400 cursor-not-allowed"}`}
                        style={inStock ? { background: "linear-gradient(135deg,#C2556A,#E8956D)" } : {}}>
                  ...
                  >
                    <div className="flex gap-4 p-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-1.5 mb-2">
                          {(item.tags ?? []).map((tag) => (
                            <span key={tag} className="text-xs font-semibold bg-[#FFF0F3] text-[#C2556A] border border-rose-100 px-2 py-0.5 rounded-full">
                              {tag}
                            </span>
                          ))}
                        </div>
                        ...
                        <div className="flex items-end justify-between flex-wrap gap-3 mt-2">
                          <div>
                            <div className="flex items-baseline gap-2 flex-wrap">
                              <span className="text-xl font-extrabold text-[#3B2A35]">₹{item.price.toLocaleString()}</span>
                              {item.originalPrice && item.originalPrice > item.price && (
                                <>
                                  <span className="text-sm text-rose-900/35 line-through">₹{item.originalPrice.toLocaleString()}</span>
                                  <span className="text-sm font-bold text-[#6B8F71]">{disc}% off</span>
                                </>
                              )}
                            </div>
                            {lowStock && <p className="text-xs font-semibold text-amber-600 mt-0.5">⚡ Only {item.stock} left!</p>}
                            {item.addedOn && <p className="text-xs text-rose-900/30 mt-0.5">{timeAgo(item.addedOn)}</p>}
                          </div>
                          ...
                          <button
                            ...
                            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-all
                              ${inStock ? "text-white hover:shadow-md hover:-translate-y-0.5" : "bg-gray-100 text-gray-400 cursor-not-allowed"}`}
                            style={inStock ? { background: "linear-gradient(135deg,#C2556A,#E8956D)" } : {}}>
                            {inStock ? <><ShoppingCart size={15} /> Add to Cart</> : "Out of Stock"}
                          </button>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex border-t border-rose-50 divide-x divide-rose-50">
                      <Link to={`/products/${item.id}`}
                        className="flex-1 flex items-center justify-center gap-1.5 py-3 text-sm font-medium text-rose-900/40 hover:text-[#C2556A] hover:bg-rose-50 transition-all">
                        <Package size={14} /> View Details
                      </Link>
                      <button onClick={() => handleRemove(item)}
                        className="flex-1 flex items-center justify-center gap-1.5 py-3 text-sm font-medium text-rose-400 hover:text-[#C2556A] hover:bg-rose-50 transition-all">
                        <Trash2 size={14} /> Remove
                      </button>
                      <Link to={`/products?category=${item.mainCategory}`}
                        className="flex-1 flex items-center justify-center gap-1.5 py-3 text-sm font-medium text-rose-900/40 hover:text-[#C2556A] hover:bg-rose-50 transition-all">
                        <Gift size={14} /> Similar Items
                      </Link>
                    </div>
        ...
        {filtered.length === 0 && (
          <div className="bg-white rounded-2xl border border-rose-100 p-12 text-center">
            ...
            <p className="text-base font-bold text-[#3B2A35] mb-1">No items in "{filter}"</p>
            <p className="text-sm text-rose-900/40">Try a different category above</p>
          </div>
        )}

        {recommendations.length > 0 && (
  <div className="mt-10">
    <h2 className="text-xl font-bold text-[#3B2A35] mb-4">
      You might also like 💝
    </h2>
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
      {recommendations.map((item) => (
        <div key={item.id}
          className="bg-white rounded-2xl border border-rose-100 p-3 shadow-sm hover:shadow-md transition-all">
          ...
          <Link to={`/products/${item.id}`}>
            ...
            />
            <p className="text-sm font-semibold text-[#3B2A35] line-clamp-2">
              {item.title}
            </p>
          </Link>

          <p className="text-xs text-rose-900/40 mb-1">
            by {item.sellerName}
          </p>

          <p className="text-sm font-bold text-[#C2556A]">
            ₹{item.price}
          </p>
        </div>
      ))}
        ...
        {items.length > 0 && (
          <div className="flex justify-center mt-6">
            <button onClick={() => dispatch(clearWishlist())}
              className="text-sm text-rose-300 hover:text-rose-500 font-medium transition-colors flex items-center gap-1.5">
              <Trash2 size={13} /> Clear entire wishlist
            </button>
          </div>
        )}
      </div>
