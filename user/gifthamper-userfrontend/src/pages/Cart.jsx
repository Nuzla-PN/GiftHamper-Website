...
import {
  Trash2, Plus, Minus, ShoppingBag, Tag, ChevronRight,
  Gift, Truck, Shield, RotateCcw, Heart, Star,
  ChevronDown, ChevronUp, Package, Sparkles, X, Info,
} from "lucide-react";
import { couponsConfig } from "../data/dataConfig";
...
function SectionLabel({ icon, label, count, color = "#C2556A", bg = "#FFF0F3", borderColor = "#F5C6D0" }) {
  return (
...
export default function CartPage() {
  ...
  const [showCouponBox, setShowCouponBox] = useState(false);
  const [wishlistIds,   setWishlistIds]   = useState([]);
  const [deliveryMode,  setDeliveryMode]  = useState("separate");

  const handleCombineHamper = () => {
    const normalItemsWithIndex = cartItems
  ...
  /* ── Seller grouping ── */
  const groupedBySeller = useMemo(() => {
    return normalItems.reduce((acc, item) => {
      const sid = item.sellerId || "unknown";
      if (!acc[sid]) acc[sid] = { sellerId: sid, sellerName: item.sellerName || "Unknown Seller", items: [] };
  ...
  const hasHampers    = hamperItems.length > 0;

  /* ── Price math ── */
  const itemsTotal    = cartItems.reduce((s, i) => s + (i.totalPrice || 0), 0);
  const originalTotal = cartItems.reduce((s, i) => s + (i.originalPrice || i.price || 0) * (i.quantity || 1), 0);
  const savedAmount   = Math.max(0, originalTotal - itemsTotal);

  const couponDiscount = appliedCoupon
    ? appliedCoupon.type === "percentage"
      ? Math.round(itemsTotal * ((appliedCoupon.discountValue || 0) / 100))
  ...
  const applyCoupon = () => {
    const found = couponsConfig.find((c) => c.code.toUpperCase() === couponInput.toUpperCase().trim());
    if (found) {
      if (itemsTotal < found.minAmount) {
        setCouponError(`Minimum order ₹${found.minAmount} required.`);
        setAppliedCoupon(null);
      } else { setAppliedCoupon(found); setCouponError(""); }
  ...
  if (isEmpty) return (
    <div className="min-h-screen bg-[#FEF9F5] flex flex-col items-center justify-center py-28 px-4 text-center">
      ...
      <h2 className="text-2xl font-bold text-[#3B2A35] mb-3">Your cart is empty 🎁</h2>
      <p className="text-base text-rose-900/50 mb-8 max-w-xs">
        Looks like you haven't added any hampers yet. Let's find something special!
      </p>
      <Link to="/products"
        className="px-10 py-3.5 rounded-full font-bold text-base text-white hover:shadow-lg hover:-translate-y-0.5 transition-all"
        style={{ background: "linear-gradient(135deg,#C2556A,#E8956D)" }}>
        Browse Hampers ✨
      </Link>
    </div>
    ...
    <div className="min-h-screen bg-[#FEF9F5]">
      ...
      <div className="bg-white border-b border-rose-100 sticky top-0 z-30 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3.5 flex items-center gap-3">
          <ShoppingBag className="w-5 h-5 text-[#C2556A]" />
          <h1 className="text-lg font-bold text-[#3B2A35]">
            My Cart
            <span className="ml-2 text-sm font-bold text-white bg-[#C2556A] rounded-full px-2.5 py-1">
              {cartItems.length} {cartItems.length === 1 ? "item" : "items"}
            </span>
          </h1>
          <Link to="/products"
            className="ml-auto text-sm text-[#C2556A] font-semibold hover:underline flex items-center gap-1">
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-5 pb-32 lg:pb-10">
        ...
        {deliveryFee > 0 ? (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
            ...
            Add <span className="font-bold mx-1 text-amber-900">₹{(FREE_DELIVERY_THRESHOLD - itemsTotal).toLocaleString()}</span> more to get FREE delivery! 🎀
          </motion.div>
        ) : (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
            className="mb-5 bg-[#F3FBF4] border border-green-200 rounded-xl px-4 py-3 flex items-center gap-2.5 text-sm font-medium text-green-800">
            <Truck size={16} className="text-green-500 shrink-0" />
            🎉 Great news! You get <span className="font-bold mx-1">FREE delivery</span> on this order!
          </motion.div>
        )}
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-start">
          ...
          <div className="flex-1 min-w-0 space-y-4">
            ...
            {cartItems.some((it) => it.stock === 0) && (
              <div className="bg-rose-50 border border-rose-200 rounded-xl px-4 py-3 flex items-start gap-2.5 text-sm text-rose-700">
                <Info size={16} className="shrink-0 mt-0.5 text-rose-400" />
                <span className="font-medium">Some items are out of stock and won't be included in your order.</span>
              </div>
            )}
              ...
              <div>
                ...
                <AnimatePresence>
                  {cartItems.map((item, globalIndex) => {
                    if (item.type !== "hamper") return null;
                    return (
                      <HamperCartCard
            ...
            {hasHampers && hasNormal && (
              <div className="flex items-center gap-3 py-1">
                <div className="flex-1 h-px bg-rose-100" />
                <span className="text-xs text-rose-300 font-semibold">also in your cart</span>
                <div className="flex-1 h-px bg-rose-100" />
              </div>
            )}
                ...
                <div className="space-y-3">
                  <AnimatePresence>
                    {cartItems.map((item, globalIndex) => {
                      ...
                      const addonTotal    = addonsArray.reduce((s, a) => s + (a?.price ?? 0), 0);
                      ...
                      return (
                        <motion.div
                          ...
                          transition={{ duration: 0.3 }}
                          className={`bg-white rounded-2xl border shadow-sm overflow-hidden
                            ${inStock ? "border-amber-100" : "border-rose-200 opacity-70"}`}
                        >
                          {/* Out of stock ribbon */}
                          {!inStock && (
                            <div className="bg-rose-50 border-b border-rose-100 px-4 py-2 text-sm font-semibold text-rose-500 flex items-center gap-2">
                              <X size={13} /> Out of Stock — not included in total
                            </div>
                          )}
                          ...
                          <div className="flex gap-4 p-4">
                            ...
                            {/* Image */}
                            <div className="relative shrink-0 w-28 h-28 sm:w-32 sm:h-32 rounded-xl overflow-hidden bg-[#FFF8EC]">
                              <img src={imgSrc} alt={item.name || item.title}
                                className="w-full h-full object-contain p-2" />
                              {disc && inStock && (
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {/* Seller */}
                              <p className="text-xs text-rose-900/40 mb-1.5">
                              ...
                              {(item.rating ?? 0) > 0 && (
                                <div className="flex items-center gap-1 mb-2">
                                  ...
                                  {(item.reviews ?? 0) > 0 && (
                                    <span className="text-xs text-rose-900/40 ml-0.5">({item.reviews})</span>
                                  )}
                                </div>
                              )}
                              ...
                              {addonsArray.length > 0 && (
                                <div className="flex flex-wrap gap-1.5 mb-2">
                                  {addonsArray.map((ad, i) => {
                                    ...
                                    return (
                                      <span key={i} className="inline-flex items-center gap-1 text-xs font-semibold bg-[#FFF0F3] text-[#C2556A] border border-rose-100 px-2.5 py-0.5 rounded-full">
                                        {label}{price ? ` +₹${price}` : ""}
                                      </span>
                                    );
                                  })}
                              ...
                              {item.customizations && Object.keys(item.customizations).length > 0 && (
                                <div className="flex flex-wrap gap-1.5 mb-2">
                                  {Object.entries(item.customizations).map(([k, v]) => v && (
                                    <span key={k} className="text-xs bg-amber-50 text-amber-700 border border-amber-100 px-2.5 py-0.5 rounded-full font-medium">
                                      {k}: {v}
                                    </span>
                                  ))}
                                </div>
                              ...
                              <div className="flex items-center justify-between flex-wrap gap-3 mt-2">
                                <div className="flex items-baseline gap-2 flex-wrap">
                                  <span className="text-lg font-extrabold text-[#3B2A35]">
                                ...
                                {/* Qty stepper */}
                                {inStock && (
                                  <div className="flex items-center border-2 border-rose-200 rounded-full overflow-hidden">
                                    <button onClick={() => dispatch(decreaseQty(globalIndex))}
                                      className="w-9 h-9 flex items-center justify-center text-[#C2556A] hover:bg-rose-50 transition-colors font-bold text-lg">
                                      <Minus size={14} strokeWidth={3} />
                                    </button>
                                    <span className="w-9 h-9 flex items-center justify-center text-sm font-extrabold text-[#3B2A35] border-x border-rose-200">
                                      {item.quantity}
                                    </span>
                                    <button onClick={() => dispatch(increaseQty(globalIndex))}
                                      className="w-9 h-9 flex items-center justify-center text-[#C2556A] hover:bg-rose-50 transition-colors">
                          ...
                          {/* Action row */}
                          <div className="flex items-center border-t border-amber-50 divide-x divide-amber-50">
                            <button onClick={() => dispatch(removeFromCart(globalIndex))}
                              className="flex-1 flex items-center justify-center gap-2 py-3 text-sm font-medium text-rose-400 hover:text-[#C2556A] hover:bg-rose-50 transition-all">
                              <Trash2 size={14} /> Remove
                            </button>
                            <button onClick={() => { toggleWishlist(item.id); dispatch(removeFromCart(globalIndex)); }}
                              className="flex-1 flex items-center justify-center gap-2 py-3 text-sm font-medium text-rose-900/40 hover:text-[#C2556A] hover:bg-rose-50 transition-all">
            ...
            {/* ── MULTI-SELLER DELIVERY MODE ── */}
            <AnimatePresence>
              {hasNormal && isMultiSeller && (
                <motion.div
                  initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                  className="bg-white rounded-2xl border border-amber-200 shadow-sm overflow-hidden"
                >
                  <div className="px-4 py-3.5 border-b border-amber-100 bg-amber-50 flex items-center gap-2.5">
                  ...
                  <div className="p-4 space-y-3">
                    {[
                      {
                        value: "separate",
                        icon: <Truck size={15} className="text-[#C2556A]" />,
                        title: "Deliver Separately",
                        badge: "FREE",
                        badgeColor: "#6B8F71",
                        badgeBg: "#F3FBF4",
                        badgeBorder: "#C6E8CD",
                        desc: "Each seller ships their items individually. Packages may arrive at different times.",
                      },
                      {
                        value: "combined",
                        icon: <Gift size={15} className="text-[#C2556A]" />,
                        title: "Combine into One Hamper",
                        badge: `+₹${COMBINE_PACKING_CHARGE} packing`,
                        badgeColor: "#C2556A",
                        badgeBg: "#FFF0F3",
                        badgeBorder: "#F5C6D0",
                        desc: "All items packed beautifully into a single gift box and delivered together 🎁",
                      },
                    ].map((opt) => (
                      <label key={opt.value}
                        className={`flex items-start gap-3.5 p-4 rounded-xl border-2 cursor-pointer transition-all
                          ${deliveryMode === opt.value ? "border-[#C2556A] bg-rose-50" : "border-rose-100 hover:border-rose-200 bg-white"}`}>
                        <input type="radio" name="deliveryMode" value={opt.value}
                          checked={deliveryMode === opt.value}
                          onChange={() => setDeliveryMode(opt.value)}
                          className="accent-[#C2556A] mt-1 shrink-0 w-4 h-4" />
                        <div className="flex-1">
                          <p className="text-sm font-bold text-[#3B2A35] flex items-center gap-2 flex-wrap">
                            {opt.icon}
                            {opt.title}
                            <span className="text-xs font-bold px-2.5 py-0.5 rounded-full border"
                              style={{ color: opt.badgeColor, background: opt.badgeBg, borderColor: opt.badgeBorder }}>
                              {opt.badge}
                            </span>
                          </p>
                          <p className="text-sm text-rose-900/50 mt-1 leading-relaxed">{opt.desc}</p>
                        </div>
                      </label>
                    ))}
                    ...
                    {deliveryMode === "combined" && (
                      <div className="flex justify-center mt-2">
                        <button onClick={handleCombineHamper}
                          className="relative overflow-hidden rounded-full font-bold text-sm text-white px-7 py-3 transition-all hover:scale-105 hover:shadow-lg"
                          style={{ background: "linear-gradient(135deg,#C2556A,#E8956D)" }}>
                          Build Combined Hamper →
                        </button>
                      </div>
                    )}
                  </div>
            ...
            <div className="bg-white rounded-2xl border border-rose-100 shadow-sm p-5">
              <button onClick={() => setShowCouponBox(!showCouponBox)}
                ...
                <span className="flex items-center gap-2.5 text-base font-bold text-[#3B2A35]">
                  <Tag size={17} className="text-[#C2556A]" />
                  Apply Coupon Code
                  {appliedCoupon && (
                    <span className="text-xs font-bold text-[#6B8F71] bg-green-50 border border-green-100 px-2.5 py-0.5 rounded-full">
                      {appliedCoupon.code} ✓
                    </span>
                  )}
                </span>
                {showCouponBox
                  ? <ChevronUp size={17} className="text-rose-300" />
                  : <ChevronDown size={17} className="text-rose-300" />}
              </button>
                  ...
                  <motion.div
                    ...
                    <div className="mt-4 flex gap-2">
                      <input value={couponInput} onChange={(e) => setCouponInput(e.target.value)}
                        onKeyDown={(e) => e.key === "Enter" && applyCoupon()}
                        placeholder="Enter coupon code"
                        className="flex-1 border border-rose-200 rounded-xl px-4 py-2.5 text-sm text-[#3B2A35] bg-[#FEF9F5] focus:outline-none focus:ring-2 focus:ring-[#C2556A]/30 focus:border-[#C2556A] placeholder:text-rose-300" />
                      <button onClick={applyCoupon}
                        className="px-6 py-2.5 rounded-xl text-sm font-bold text-white transition-all hover:shadow-md"
                        style={{ background: "linear-gradient(135deg,#C2556A,#E8956D)" }}>
                        Apply
                      </button>
                    </div>
                    {couponError && (
                      <p className="text-sm text-rose-400 mt-2 flex items-center gap-1.5">
                        <X size={13} /> {couponError}
                      </p>
                    )}
                    {appliedCoupon && !couponError && (
                      <p className="text-sm text-[#6B8F71] font-semibold mt-2">
                        ✓ Coupon applied! You save ₹{couponDiscount}
                      </p>
                    )}
                    {couponsConfig.length > 0 && (
                      <div className="mt-4 space-y-2">
                        <p className="text-xs text-rose-900/40 font-bold uppercase tracking-wide">Available Coupons</p>
                        {couponsConfig.map((c) => (
                          <button key={c.id}
                            onClick={() => { setCouponInput(c.code); setAppliedCoupon(c); setCouponError(""); }}
                            className={`w-full flex items-center justify-between px-4 py-3 rounded-xl border text-left transition-all
                              ${appliedCoupon?.id === c.id ? "border-[#C2556A] bg-rose-50" : "border-rose-100 bg-[#FEF9F5] hover:border-[#E8956D]"}`}>
                            <div>
                              <p className="text-sm font-bold text-[#C2556A]">{c.code}</p>
                              <p className="text-xs text-rose-900/50 mt-0.5">{c.description}</p>
                            </div>
                            {appliedCoupon?.id === c.id
                              ? <span className="text-xs font-bold text-[#6B8F71] bg-green-50 px-2.5 py-1 rounded-full shrink-0">Applied ✓</span>
                              : <span className="text-xs font-bold text-[#C2556A] shrink-0">Tap to apply</span>}
                          </button>
                        ))}
                      </div>
                    )}
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <div key={label} className="bg-white rounded-xl border border-rose-50 p-3.5 text-center shadow-sm">
                  <div className="w-9 h-9 rounded-full mx-auto mb-2 flex items-center justify-center"
                    style={{ background: "linear-gradient(135deg,#FFF0F3,#FFF6EC)" }}>
                    <Icon size={16} className="text-[#C2556A]" />
                  </div>
                  <p className="text-xs font-bold text-[#3B2A35]">{label}</p>
                  <p className="text-[10px] text-rose-900/40 mt-0.5">{sub}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="w-full lg:w-80 xl:w-96 shrink-0 space-y-3 lg:sticky lg:top-20">
            ...
            <div className="bg-white rounded-2xl border border-rose-100 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-rose-50 flex items-center gap-2.5"
                style={{ background: "linear-gradient(90deg,#FFF0F3,#FFF6EC)" }}>
                <Sparkles size={16} className="text-[#C2556A]" />
                <h2 className="text-base font-bold text-[#3B2A35]">Order Summary</h2>
              </div>

              <div className="px-5 py-5 space-y-3.5 text-sm">

                {/* Normal items subtotal */}
                {hasNormal && (
                  <div className="space-y-1.5">
                    <p className="text-xs font-bold text-rose-900/40 uppercase tracking-widest flex items-center gap-1.5">
                      <Package size={11} /> Products ({normalItems.length})
                    </p>
                    {normalItems.map((it, i) => {
                      const addA = Array.isArray(it.addons) ? it.addons
                        : it.addons && typeof it.addons === "object" ? Object.values(it.addons).filter(Boolean) : [];
                      const adA  = addA.reduce((s, a) => s + (a?.price ?? 0), 0);
                      return (
                        <div key={i} className="flex justify-between text-sm text-rose-900/60">
                          <span className="line-clamp-1 flex-1 pr-2">
                            {it.name || it.title}
                            {(it.quantity ?? 1) > 1 && <span className="text-rose-900/40"> ×{it.quantity}</span>}
                          </span>
                          <span className="font-semibold text-[#3B2A35] shrink-0">
                            ₹{(it.totalPrice ?? ((it.price ?? 0) * (it.quantity ?? 1) + adA * (it.quantity ?? 1))).toLocaleString()}
                          </span>
                        </div>
                      );
                    })}
                ...
                {hasHampers && (
                  <div className="space-y-1.5">
                    {hasNormal && <div className="border-t border-rose-50 pt-2" />}
                    <p className="text-xs font-bold text-rose-900/40 uppercase tracking-widest flex items-center gap-1.5">
                      <Gift size={11} /> Custom Hampers ({hamperItems.length})
                    </p>
                    {hamperItems.map((it, i) => (
                      <div key={i} className="flex justify-between text-sm text-rose-900/60">
                        <span className="line-clamp-1 flex-1 pr-2">
                          {it.box?.name || "Custom Hamper"}
                          <span className="text-rose-900/40"> · {it.items?.length ?? 0} items</span>
                        </span>
                        <span className="font-semibold text-[#3B2A35] shrink-0">
                ...
                <div className="border-t border-rose-100 pt-3 space-y-3">
                  <div className="flex justify-between text-sm text-rose-900/60">
                    <span>Subtotal ({cartItems.length} {cartItems.length === 1 ? "item" : "items"})</span>
                    <span className="font-semibold text-[#3B2A35]">₹{originalTotal.toLocaleString()}</span>
                  </div>
                  {savedAmount > 0 && (
                    <div className="flex justify-between text-sm text-[#6B8F71]">
                      <span>Discount</span>
                      <span className="font-semibold">-₹{savedAmount.toLocaleString()}</span>
                    </div>
                  )}
                  ...
                  {packingCharge > 0 && (
                    <div className="flex justify-between text-sm text-rose-900/60">
                      <span className="flex items-center gap-1.5"><Gift size={12} className="text-[#C2556A]" /> Packing Charge</span>
                      <span className="font-semibold text-[#3B2A35]">₹{packingCharge}</span>
                    </div>
                  )}
                  <div className="flex justify-between text-sm text-rose-900/60">
                    <span>Delivery</span>
                    {deliveryFee === 0
                      ? <span className="font-semibold text-[#6B8F71]">FREE 🎉</span>
                      : <span className="font-semibold text-[#3B2A35]">₹{deliveryFee}</span>}
                  </div>
                </div>

                <div className="border-t-2 border-rose-100 pt-4 flex justify-between items-center">
                  <span className="text-base font-bold text-[#3B2A35]">Total Amount</span>
                  <span className="text-xl font-extrabold text-[#C2556A]">₹{grandTotal.toLocaleString()}</span>
                </div>

                {(savedAmount + couponDiscount) > 0 && (
                  <div className="bg-[#F3FBF4] border border-green-100 rounded-xl px-3 py-2.5 text-sm font-semibold text-[#6B8F71] text-center">
                    🎉 You're saving ₹{(savedAmount + couponDiscount).toLocaleString()} on this order!
                  </div>
                )}
              </div>
            ...
            <div className="bg-white rounded-2xl border border-rose-100 shadow-sm p-4 flex items-center gap-3">
              <div className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0"
                style={{ background: "linear-gradient(135deg,#FFF0F3,#FFF6EC)" }}>
                <Gift size={20} className="text-[#C2556A]" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-[#3B2A35]">Add Gift Wrapping 🎀</p>
                <p className="text-xs text-rose-900/40 mt-0.5">Make it extra special</p>
              </div>
              <button className="shrink-0 text-sm font-bold text-[#C2556A] border border-rose-200 rounded-full px-3.5 py-1.5 hover:bg-rose-50 transition-colors">
                + ₹49
              </button>
            </div>

            <button onClick={() => dispatch(clearCart())}
              className="w-full text-sm text-rose-300 hover:text-rose-400 font-medium py-2 transition-colors">
              Clear entire cart
            </button>
          </div>
        </div>
      ...
      {/* ── MOBILE STICKY CHECKOUT BAR ── */}
      <div className="fixed bottom-0 left-0 right-0 z-40 lg:hidden border-t border-rose-100 shadow-lg"
        style={{ background: "rgba(254,249,245,0.97)", backdropFilter: "blur(12px)" }}>
        <div className="px-4 py-3.5 flex items-center gap-3">
          <div>
            <p className="text-xs text-rose-900/40 font-medium">Total Amount</p>
            <p className="text-lg font-extrabold text-[#3B2A35]">₹{grandTotal.toLocaleString()}</p>
          </div>
          {(savedAmount + couponDiscount) > 0 && (
            <span className="text-xs font-semibold text-[#6B8F71] bg-green-50 border border-green-100 px-2.5 py-1 rounded-full">
              Save ₹{(savedAmount + couponDiscount).toLocaleString()}
            </span>
          )}
          <Link to="/checkout"
