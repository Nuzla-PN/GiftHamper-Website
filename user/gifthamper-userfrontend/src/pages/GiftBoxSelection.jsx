...
// import { setGiftBox } from "../features/addson/addsonSlice";
// import { giftBoxOptionsConfig } from "../data/dataConfig";

// export default function GiftBoxSelection() {
//   const navigate = useNavigate();
//   const dispatch= useDispatch();
//   const { id: productId } = useParams();
//  const giftBoxFromRedux = useSelector((state) => state.addons.giftBox);

//   const [selectedBox, setSelectedBox] = useState(giftBoxFromRedux || null);
...
//   const handleApply = () => {
//   const selectedOption = giftBoxOptionsConfig.find(
//     (box) => box.id === selectedBox
//   );

//   if (selectedOption) {
//     dispatch(setGiftBox(selectedOption)); 
//   }
...
// const handleSkip = () => {
//  dispatch(setGiftBox(null)); // clear gift box
//   navigate(`/products/${productId}`);
// };
...
export default function GiftBoxSelection() {
  ...
  const [selectedBox, setSelectedBox] = useState(giftBoxFromRedux?.id || null);

  const handleSelect = (boxId) => setSelectedBox(boxId);

  const handleApply = () => {
    const selectedOption = giftBoxOptionsConfig.find((box) => box.id === selectedBox);
    if (selectedOption) {
      dispatch(setGiftBox(selectedOption));
    }
    navigate(`/products/${productId}`);
  ...
  const handleSkip = () => {
    dispatch(setGiftBox(null));
    navigate(`/products/${productId}`);
  };

  const activeBox = giftBoxOptionsConfig.find((b) => b.id === selectedBox);
    ...
    <div className="min-h-screen bg-[#faf8f6] pb-24">
      ...
      >
        ...
        <div className="relative z-10 mt-4 flex items-end justify-between gap-3 flex-wrap">
          ...
          >
            <Gift size={14} />
            {giftBoxOptionsConfig.length} boxes available
          </div>
        </div>
      </div>
            ...
            return (
              ...
              >
                ...
                <div className="p-4 flex flex-col gap-1.5">
                  <h3 className="font-semibold text-gray-800 text-base">{box.name}</h3>
                ...
                <div className="px-4 pb-4">
                  <button
                    onClick={(e) => { e.stopPropagation(); handleSelect(box.id); }}
                    className={`
